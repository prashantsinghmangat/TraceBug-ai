...
- **Unbounded rrweb buffer during long recordings (OOM risk)** (`src/rrweb-recorder.ts`) — the DOM-replay event array had no cap, so a multi-hour "record all day" session could accumulate hundreds of MB and crash the tab before export. rrweb now emits a fresh full snapshot every ~2 minutes (`checkoutEveryNms`), and the buffer is trimmed back to the most recent full snapshot past a soft cap (with a hard ceiling as a floor guarantee) — the retained tail always stays replayable. `wasDomBufferTrimmed()` reports when a session was windowed. Pure `trimEventBuffer()` is unit-tested.
- **Console/error messages could bloat the report (and drop on circular refs)** (`src/collectors.ts`) — captured console arguments and error messages/stacks are now capped at 10 000 chars (`…[truncated]`), so a single `console.error("x", hugeObject)` can't retain a multi-MB string in the event buffer and localStorage. The new `formatConsoleArgs` is also circular-safe: a logged circular object degrades to `[unserializable]` instead of throwing and silently dropping the whole event.
- **Quick Bug modal now traps focus** (`src/ui/quick-bug.ts`) — Tab / Shift+Tab cycle within the dialog instead of escaping to the inert page behind it (WCAG 2.4.3 / 2.1.2). Escape-to-close and focus-restore-on-close were already present; this completes the modal's keyboard semantics. `trapModalTab` / `getFocusableElements` are unit-tested.
- **CI now runs the browser e2e suite** (`.github/workflows/ci.yml`) — an `e2e` job builds the real SDK bundle and runs the headless Playwright checks (blur tracking, pre-record flow, inspect mode) so regressions in the most complex recorded flows are caught automatically. The e2e scripts are now path-portable (were hardcoded to a local absolute path).
- **Unified sensitive-query-param detection** (`src/url-hygiene.ts` `isSensitiveParamName`) — the capture-time URL redactor and the upload sanitizer kept two separate key lists that had drifted (one caught `sig`/`signature`, the other `session`/`sid`/`csrf`/`pwd`; each under-redacted what the other caught). Both now share one matcher that is a strict superset of both — neither path ever redacts less than before.
- **Faster source-map resolution** (`cli/source-map.ts`) — a map's `mappings` are now decoded once and cached (per map object), and the map-file path cache is module-scoped across calls. Previously every stack frame re-decoded every mapping line up to its target and re-walked the repo for the bundle — seconds on a large map with a deep repo; now one decode + an O(segments-in-line) lookup. `clearSourceMapCache()` added for long-lived servers.
- **Light-mode muted text meets WCAG AA** (`src/theme.ts`) — `--tb-text-muted` was `#82828C` (3.8:1 on white, fails AA for normal text); now `#75757E` (4.56:1).
- **Long standard recordings warn past 1 GB** (`tracebug-extension/offscreen.js`) — a multi-hour non-rolling recording accumulates all chunks in the offscreen document; it now logs a one-time warning past 1 GB advising rolling mode (the default), rather than silently growing. No hard cap — truncating the video could lose the bug.
- **Unified "is this our UI element?" check for the mode tools** (`src/dom-helpers.ts`) — annotate, blur, and inspect each carried a near-identical ancestor walk with drifted prefix sets; they now share one `isTraceBugUiElement`. Deliberately matches only widget prefixes (`tracebug-`/`bt-` ids, `tracebug-`/`tb-qb`/`tb-hud`/`tb-rs` classes, `data-tracebug`) and **not** bare `tb-`, because `tb-mask`/`tb-block` are redaction markers applied to host-page content — a broad match would treat a blurred element as our own UI and break second-click-to-unblur (caught by e2e, locked in by a test). The capture-path check in collectors is intentionally left separate — broadening it could drop host events.
- **`init()` split into named phases** (`src/index.ts`) — the ~250-line `init()` was extracted into six private phase methods (`_validateConfig`, `_applyThemeFromConfig`, `_wireQuickBugConfig`, `_buildEmit`, `_startCollectors`, `_mountDashboardIfEnabled`, `_restoreVideoSessionOnInit`) with the orchestrator now ~80 lines. Pure internal refactor — no public-API or behavior change (verified: 303 unit tests + all e2e green). The `TraceBugSDK` facade split flagged in the audit stays deferred to a major version.
