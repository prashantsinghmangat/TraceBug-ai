...
var TraceBugModule = (() => {
  ...
  async function getHtml2Canvas() {
    if (!_html2canvas) {
      try {
        const mod = await Promise.resolve().then(() => (init_html2canvas_esm(), html2canvas_esm_exports));
...
`;
  ...
  function _refreshPersistentBadges(root) {
    ...
    `;
      ...
      badge.title = `${a2.intent.toUpperCase()}: ${a2.comment.slice(0, 60)}`;
      badge.style.cssText = `
  ...
  // src/onboarding.ts
  var STORAGE_KEY = "tracebug_onboarding_complete";
  ...
  function mountCompactToolbar(root, panel, showToast2, renderAnnotationList2, position2 = "right") {
    _panelEl = panel;
    _position = position2;
    ...
    toolbar.appendChild(_createToolbarBtn(
      "TraceBug Dashboard",
      _logoSvg(),
      () => _togglePanel(panel, toolbar, showToast2),
      ...
      () => {
        if (isElementAnnotateActive()) {
          ...
          activateElementAnnotateMode(root, () => {
            ...
            showToast2("Annotation saved", root);
          }, () => {
            _updateActiveStates(toolbar);
          });
          _updateActiveStates(toolbar);
    ...
    const listBtn = _createToolbarBtn(
      ...
      () => {
        if (_annotationViewOpen && _panelOpen) {
          ...
          if (!_panelOpen) _togglePanel(panel, toolbar, showToast2);
  ...
  `;
    ...
    toolbar.appendChild(_createToolbarBtn(
      ...
      (e2) => _toggleSettingsCard(e2, root, toolbar, showToast2),
    ...
    root.appendChild(toolbar);
    if (_isMobile) {
      _convertToFab(toolbar, root, panel, showToast2);
    }
    const resizeHandler = () => {
    ...
    const keyHandler = (e2) => {
      var _a, _b;
      if (e2.ctrlKey && e2.shiftKey && e2.key === "A") {
        e2.preventDefault();
  ...
  function _togglePanel(panel, toolbar, showToast2) {
  ...
  function _toggleSettingsCard(e2, root, toolbar, showToast2) {
    e2.stopPropagation();
  ...
  `;
    ...
    card.querySelector("#tracebug-settings-clear-ann").addEventListener("click", () => {
      clearAllAnnotations();
      _updateAnnotationCount(toolbar);
      showToast2("Annotations cleared", root);
      card.remove();
    });
    card.querySelector("#tracebug-settings-clear-all").addEventListener("click", () => {
      if (confirm("Delete all TraceBug data?")) {
        ...
        clearAllAnnotations();
        _updateAnnotationCount(toolbar);
        showToast2("All data cleared", root);
        card.remove();
      }
    });
  ...
  function _convertToFab(toolbar, root, panel, showToast2) {
  ...
  function mountDashboard(toolbarPosition) {
  ...
  function _createSessionCard(session, panel) {
    ...
    <div style="display:flex;align-items:center;justify-content:space-between">
      <div style="display:flex;align-items:center">
        ${dot}
        <span style="color:var(--tb-text-primary, #e0e0e0);font-size:12px;font-weight:600">${escapeHtml2(sessionName)}</span>
        ${badge}
      </div>
      <span style="color:var(--tb-text-muted, #555);font-size:10px">${timeAgo(session.updatedAt)}</span>
    </div>
    <div style="color:var(--tb-text-muted, ${hasError ? "#f87171" : "#888"});font-size:11px;margin-top:6px;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml2(preview)}</div>
  ...
  function renderSessionDetail(panel, session) {
    ...
    <div style="font-size:14px;font-weight:700;color:var(--tb-text-primary, #fff);font-family:var(--tb-font-family, system-ui,sans-serif);line-height:1.3;margin-bottom:10px">${escapeHtml2(bugTitle)}</div>
    ...
    if (problems.length > 0) {
      ...
      for (const p of problems) {
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
          ...
          <span style="font-size:11px;font-weight:600;color:${p.color};font-family:var(--tb-font-family, system-ui,sans-serif)">${escapeHtml2(p.title)}</span>
        </div>
        <div style="color:var(--tb-text-muted, #888);font-size:11px;line-height:1.4;padding-left:22px;word-break:break-word">${escapeHtml2(p.detail)}</div>
      </div>`;
      }
      html += `</div>`;
    }
      ...
      if (s.errorStack) {
        ...
        if (locationMatch) {
          ...
          <div style="display:flex;align-items:center;gap:6px">
            ...
            <span style="font-size:10px;color:var(--tb-text-muted, #888);word-break:break-all">${escapeHtml2(locationMatch[2])}</span>
          </div>
        </div>`;
        }
        ...
        <pre style="color:#dc262690;font-size:10px;margin-top:6px;white-space:pre-wrap;line-height:1.4;max-height:150px;overflow:auto;background:#0a0a0a;padding:8px;border-radius:var(--tb-radius-sm, 4px)">${escapeHtml2(s.errorStack)}</pre>
      </details>`;
      }
      html += `</div>`;
    }
      ...
      for (const ev of apiEvents) {
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
          <div style="display:flex;align-items:center;gap:6px">
            ...
            <span style="font-size:10px;color:var(--tb-text-muted, #888);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:180px">${escapeHtml2(urlPath)}</span>
          </div>
          <div style="display:flex;align-items:center;gap:6px">
    ...
    if (annotations.length > 0) {
      ...
      for (const note of annotations) {
        ...
        <div style="color:var(--tb-text-primary, #e0e0e0);font-size:12px;line-height:1.4">${escapeHtml2(note.text)}</div>
        ${note.expected ? `<div style="margin-top:4px;font-size:11px"><span style="color:var(--tb-success, #22c55e);font-weight:600">Expected:</span> <span style="color:var(--tb-text-secondary, #aaa)">${escapeHtml2(note.expected)}</span></div>` : ""}
        ${note.actual ? `<div style="margin-top:2px;font-size:11px"><span style="color:var(--tb-error, #ef4444);font-weight:600">Actual:</span> <span style="color:var(--tb-text-secondary, #aaa)">${escapeHtml2(note.actual)}</span></div>` : ""}
      </div>`;
      }
      html += `</div>`;
    }
    ...
    if (screenshots2.length > 0) {
      ...
      for (const ss of screenshots2) {
        ...
        <div style="font-size:10px;color:var(--tb-text-muted, #888);margin-bottom:4px">${escapeHtml2(ss.filename)} \u2014 ${escapeHtml2(ss.page)}</div>
    ...
    if (envInfo) {
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
        <div style="background:var(--tb-bg-primary, #0f0f1a);border-radius:var(--tb-radius-md, 6px);padding:6px 8px"><span style="font-size:9px;color:var(--tb-text-muted, #555)">Browser</span><div style="font-size:12px;color:var(--tb-text-primary, #e0e0e0)">${escapeHtml2(envInfo.browser)} ${escapeHtml2(envInfo.browserVersion)}</div></div>
        <div style="background:var(--tb-bg-primary, #0f0f1a);border-radius:var(--tb-radius-md, 6px);padding:6px 8px"><span style="font-size:9px;color:var(--tb-text-muted, #555)">OS</span><div style="font-size:12px;color:var(--tb-text-primary, #e0e0e0)">${escapeHtml2(envInfo.os)}</div></div>
        <div style="background:var(--tb-bg-primary, #0f0f1a);border-radius:var(--tb-radius-md, 6px);padding:6px 8px"><span style="font-size:9px;color:var(--tb-text-muted, #555)">Viewport</span><div style="font-size:12px;color:var(--tb-text-primary, #e0e0e0)">${escapeHtml2(envInfo.viewport)}</div></div>
      ...
      if (ev.type === "api_request") {
        ...
        html += `<div style="margin-top:4px">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
            ...
            <span style="font-size:11px;color:var(--tb-text-secondary, #aaa);word-break:break-all">${escapeHtml2((urlPath == null ? void 0 : urlPath.slice(0, 80)) || "")}</span>
          </div>
        ...
        html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">Clicked "<span style="color:#60a5fa">${escapeHtml2(target.slice(0, 60))}</span>"</div>`;
        const details = [];
        ...
        const inputType = (inp == null ? void 0 : inp.type) || "text";
        if (inputType === "checkbox" || inputType === "radio") {
          html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">${(inp == null ? void 0 : inp.checked) ? "Checked" : "Unchecked"} "<span style="color:#c084fc">${escapeHtml2(fieldName)}</span>" <span style="font-size:9px;color:var(--tb-text-muted, #555)">(${inputType})</span></div>`;
        } else {
          const val = inp == null ? void 0 : inp.value;
          if (val && val !== "[REDACTED]") {
            html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">Typed in "<span style="color:#c084fc">${escapeHtml2(fieldName)}</span>" <span style="font-size:9px;color:var(--tb-text-muted, #555)">(${inputType})</span></div>`;
            html += `<div style="font-size:10px;color:#a78bfa;margin-top:3px;background:#1e153344;padding:3px 8px;border-radius:var(--tb-radius-sm, 4px);border:1px solid #1e1533;word-break:break-word">"${escapeHtml2(val.slice(0, 150))}"</div>`;
          } else {
            html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">Typed in "<span style="color:#c084fc">${escapeHtml2(fieldName)}</span>" <span style="font-size:9px;color:var(--tb-text-muted, #555)">(${inputType})</span></div>
        ...
        if (inp == null ? void 0 : inp.placeholder) {
          html += `<div style="font-size:9px;color:var(--tb-text-muted, #333);margin-top:2px">placeholder: "${escapeHtml2(inp.placeholder)}"</div>`;
        }
      } else if (ev.type === "select_change") {
        const sel = ev.data.element;
        const fieldName = (sel == null ? void 0 : sel.name) || (sel == null ? void 0 : sel.id) || "dropdown";
        html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">Changed "<span style="color:#34d399">${escapeHtml2(fieldName)}</span>" dropdown</div>`;
        html += `<div style="font-size:11px;color:#34d399;margin-top:3px;background:#05201544;padding:4px 8px;border-radius:var(--tb-radius-sm, 4px);border:1px solid #14532d">Selected: "<strong>${escapeHtml2((sel == null ? void 0 : sel.selectedText) || (sel == null ? void 0 : sel.value) || "")}</strong>"</div>`;
        if ((sel == null ? void 0 : sel.allOptions) && sel.allOptions.length > 0) {
          html += `<div style="font-size:9px;color:var(--tb-text-muted, #444);margin-top:3px">Options: ${sel.allOptions.map((o) => escapeHtml2(o)).join(", ")}</div>`;
        }
      } else if (ev.type === "form_submit") {
        const f2 = ev.data.form;
        html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.4">Submitted form ${(f2 == null ? void 0 : f2.id) ? `"<span style="color:#fb923c">${escapeHtml2(f2.id)}</span>"` : ""}</div>`;
        if ((f2 == null ? void 0 : f2.fields) && Object.keys(f2.fields).length > 0) {
          ...
          for (const [key, val] of Object.entries(f2.fields)) {
            html += `<div style="font-size:10px;margin-bottom:2px"><span style="color:var(--tb-text-muted, #888)">${escapeHtml2(key)}:</span> <span style="color:var(--tb-warning, #fbbf24)">${escapeHtml2(String(val).slice(0, 80))}</span></div>`;
          }
          html += `</div>`;
        }
        if (f2 == null ? void 0 : f2.method) {
          html += `<div style="font-size:9px;color:var(--tb-text-muted, #444);margin-top:2px">${f2.method.toUpperCase()} ${f2.action ? `\u2192 ${escapeHtml2(f2.action.slice(0, 60))}` : ""}</div>`;
        }
      } else if (ev.type === "route_change") {
        html += `<div style="display:flex;align-items:center;gap:6px;font-size:11px;margin-top:2px">
          <span style="color:var(--tb-text-muted, #888);background:var(--tb-bg-primary, #0f0f1a);padding:2px 6px;border-radius:3px">${escapeHtml2(ev.data.from || "/")}</span>
        ...
        html += `<div style="margin-top:2px">
          ...
          ${((_l = ev.data.error) == null ? void 0 : _l.source) ? `<div style="font-size:9px;color:var(--tb-text-muted, #555);margin-top:3px">at ${escapeHtml2(ev.data.error.source)}${ev.data.error.line ? `:${ev.data.error.line}` : ""}${ev.data.error.column ? `:${ev.data.error.column}` : ""}</div>` : ""}
        </div>`;
        ...
        html += `<div style="color:var(--tb-text-secondary, #aaa);font-size:11px;line-height:1.3">${escapeHtml2(describeEvent(ev))}</div>`;
      }
      html += `</div></div>`;
    }
    html += `</div>`;
...
<div class="meta">Session: ${s.sessionId} \xB7 ${new Date(s.createdAt).toLocaleString()} \xB7 Duration: ${formatDuration(sessionDur)}</div>`;
    ...
    if (problems.length > 0) {
      ...
      for (const p of problems) {
        ...
        <div style="color:var(--tb-text-muted, #888);font-size:12px;padding-left:24px">${escapeHtml2(p.detail)}</div>
      </div>`;
      }
    }
    if (s.errorMessage) {
      ...
      ${s.errorStack ? `<pre style="color:#dc262690;font-size:11px;background:#0a0a0a;padding:10px;border-radius:var(--tb-radius-md, 6px);max-height:200px;overflow:auto">${escapeHtml2(s.errorStack)}</pre>` : ""}
    </div>`;
    }
    if (s.reproSteps) {
...
<div style="text-align:center;color:var(--tb-text-muted, #333);font-size:11px;margin-top:32px;padding:16px;border-top:1px solid #1e1e32">Generated by TraceBug AI \xB7 ${(/* @__PURE__ */ new Date()).toLocaleString()}</div>
  ...
  function describeEventHtml(ev) {
    ...
    switch (ev.type) {
      case "click": {
        ...
        if (el == null ? void 0 : el.href) s += ` <span style="font-size:10px;color:var(--tb-text-muted, #444)">\u2192 ${escapeHtml2(el.href.slice(0, 60))}</span>`;
        return s;
      }
      case "input": {
        const inp = ev.data.element;
      ...
      case "form_submit": {
        ...
        if (f2 == null ? void 0 : f2.fields) {
          ...
          for (const [key, val] of Object.entries(f2.fields)) {
            s += `<div style="font-size:10px"><span style="color:var(--tb-text-muted, #888)">${escapeHtml2(key)}:</span> ${escapeHtml2(String(val).slice(0, 80))}</div>`;
          }
          s += `</div>`;
        }
        return s;
      ...
      case "route_change":
        return `<span style="color:var(--tb-text-muted, #888)">${escapeHtml2(ev.data.from || "/")}</span> <span style="color:#22d3ee">\u2192</span> <span style="color:#22d3ee;font-weight:600">${escapeHtml2(ev.data.to || "/")}</span>`;
      case "api_request": {
        const r = ev.data.request;
        return `<span style="font-weight:600">${r == null ? void 0 : r.method}</span> ${escapeHtml2(((r == null ? void 0 : r.url) || "").slice(0, 80))} \u2192 <span style="color:${getStatusColor((r == null ? void 0 : r.statusCode) || 0)};font-weight:700">${r == null ? void 0 : r.statusCode}</span> <span style="color:var(--tb-text-muted, #555)">(${r == null ? void 0 : r.durationMs}ms)</span>`;
      }
      case "error":
      case "unhandled_rejection":
  ...
  `;
    ...
    if (total > 0) {
      (_a = panel.querySelector("#bt-ann-export-md")) == null ? void 0 : _a.addEventListener("click", async () => {
      ...
      (_b = panel.querySelector("#bt-ann-export-json")) == null ? void 0 : _b.addEventListener("click", async () => {
      ...
      (_c = panel.querySelector("#bt-ann-clear-all")) == null ? void 0 : _c.addEventListener("click", () => {
    ...
    if (elAnnotations.length > 0) {
      ...
      for (let i = 0; i < elAnnotations.length; i++) {
        ...
        <div style="font-size:11px;color:#777;margin-bottom:6px">
          <span style="background:#1e1e32;padding:1px 6px;border-radius:3px;font-family:monospace;font-size:10px">&lt;${escapeHtml2(a2.tagName)}&gt;</span>
          ${a2.innerText ? `<span style="color:#999;margin-left:4px">"${escapeHtml2(a2.innerText.slice(0, 40))}"</span>` : ""}
        </div>
        <div style="font-size:12px;color:var(--tb-text-primary, #e0e0e0);line-height:1.5">${escapeHtml2(a2.comment)}</div>
      </div>`;
      }
    }
    if (drawRegions.length > 0) {
      ...
      for (let i = 0; i < drawRegions.length; i++) {
        ...
        <div style="font-size:12px;color:var(--tb-text-primary, #e0e0e0);line-height:1.5">${r.comment ? escapeHtml2(r.comment) : '<span style="color:var(--tb-text-muted, #555);font-style:italic">No comment added</span>'}</div>
      </div>`;
      }
    }
    content2.innerHTML = html;
  ...
  function isTraceBugElement(el) {
    if (!el) return false;
    if (el.id === ROOT_ID || el.id === BTN_ID2 || el.id === PANEL_ID3) return true;
    ...
    OrigXHR.prototype.send = function(body) {
      try {
        ...
        xhr.addEventListener("loadend", function() {
          try {
            emit("api_request", { request: { url: url.slice(0, 500), method: method.toUpperCase(), statusCode: xhr.status, durationMs: Date.now() - start } });
          } catch (e2) {
          }
        });
        xhr.addEventListener("error", function() {
          try {
            emit("api_request", { request: { url: url.slice(0, 500), method: method.toUpperCase(), statusCode: 0, durationMs: Date.now() - start } });
          } catch (e2) {
          }
        });
      } catch (err) {
