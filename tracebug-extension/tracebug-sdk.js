...
var TraceBugModule = (() => {
  ...
  function getHtml2Canvas() {
    return typeof html2canvas_esm_default === "function" ? html2canvas_esm_default : null;
  }
    ...
    toolbar.appendChild(_createToolbarBtn(
      ...
      async () => {
        ...
        try {
          ...
          showToast2(`Screenshot saved: ${ss.filename}`, root);
        } catch (e2) {
...
<div style="text-align:center;color:var(--tb-text-muted, #333);font-size:11px;margin-top:32px;padding:16px;border-top:1px solid #1e1e32">Generated by TraceBug AI \xB7 ${(/* @__PURE__ */ new Date()).toLocaleString()}</div>
  ...
  `;
    ...
    if (screenshotsList.length > 0) {
      ...
      for (const ss of screenshotsList) {
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
          <span style="font-size:12px;color:var(--tb-text-primary, #e0e0e0);font-weight:600">${escapeHtml2(ss.filename)}</span>
        ...
        <img src="${ss.dataUrl}" style="max-width:100%;border-radius:var(--tb-radius-md, 6px);border:1px solid var(--tb-border, #2a2a3e)" alt="${escapeHtml2(ss.filename)}" />
      </div>`;
      }
    }
    content2.innerHTML = html;
  ...
  function isTraceBugElement(el) {
    if (!el) return false;
    if (el.id === ROOT_ID || el.id === BTN_ID || el.id === PANEL_ID2) return true;
