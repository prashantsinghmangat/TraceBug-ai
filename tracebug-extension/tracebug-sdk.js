...
`;
  ...
  function _intentBtnStyle(intent, active) {
    ...
    if (active) {
      return `background:${color2}33;color:${color2};border:1px solid ${color2};border-radius:8px;padding:6px 11px;cursor:pointer;font-size:11px;font-weight:600;font-family:inherit;transition:all 0.15s;`;
    }
  ...
  `;
    ...
    if (elAnnotations.length > 0) {
      ...
      for (let i = 0; i < elAnnotations.length; i++) {
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
          ...
          <span style="font-size:10px;padding:2px 8px;border-radius:4px;background:${intentColor}22;color:${intentColor};border:1px solid ${intentColor}44;font-weight:600;text-transform:uppercase">${a2.intent}</span>
          <span style="font-size:10px;padding:2px 8px;border-radius:4px;background:${sevColor}15;color:${sevColor};font-weight:500">${a2.severity}</span>
          <span style="font-size:10px;color:#555;margin-left:auto">${ago}</span>
          <button class="bt-ann-delete" data-id="${a2.id}" style="background:#ef444415;border:1px solid #ef444433;color:#ef4444;cursor:pointer;font-size:11px;padding:3px 8px;border-radius:4px;font-family:inherit" title="Delete this annotation">Delete</button>
        </div>
        ...
        <div style="font-size:12px;color:#e0e0e0;line-height:1.5">${escapeHtml2(a2.comment)}</div>
      </div>`;
      }
    }
    if (drawRegions.length > 0) {
      ...
      for (let i = 0; i < drawRegions.length; i++) {
        ...
        <div style="font-size:12px;color:#e0e0e0;line-height:1.5">${r.comment ? escapeHtml2(r.comment) : '<span style="color:#555;font-style:italic">No comment added</span>'}</div>
      </div>`;
      }
    }
    content2.innerHTML = html;
