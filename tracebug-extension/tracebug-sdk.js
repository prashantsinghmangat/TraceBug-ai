...
`;
  ...
  function activateElementAnnotateMode(root, onUpdate) {
    if (_active) return;
    _active = true;
  ...
  function _intentBtnStyle(intent, active) {
    ...
    if (active) {
      return `background:${color2}33;color:${color2};border:1px solid ${color2};border-radius:6px;padding:4px 10px;cursor:pointer;font-size:11px;font-weight:600;font-family:inherit;`;
    }
  ...
  var _onUpdate2 = null;
  var COLORS2 = ["#7B61FF", "#ef4444", "#eab308", "#22c55e", "#3b82f6"];
  var MAX_CANVAS_DIM = 32767;
  function isDrawModeActive() {
    return _active2;
  }
  function activateDrawMode(root, onUpdate) {
    if (_active2) return;
    _active2 = true;
  ...
  `;
    ...
    for (const s of shapes) {
      ...
      btn.addEventListener("click", (e2) => {
        ...
        bar.querySelectorAll("[data-tracebug='draw-tool-btn']").forEach((b) => {
          var _a;
          const isActive = b.title === ((_a = shapes.find((sh) => sh.shape === _currentShape)) == null ? void 0 : _a.label);
          b.style.cssText = _toolBtnStyle(isActive || false);
        });
      });
      bar.appendChild(btn);
    }
    ...
    for (const color2 of COLORS2) {
      ...
      background: ${color2}; border: 2px solid ${color2 === _currentColor ? "#fff" : color2};
      padding: 0; margin: 0;
    `;
      btn.addEventListener("click", (e2) => {
        e2.stopPropagation();
        ...
        bar.querySelectorAll("[data-tracebug='draw-color-btn']").forEach((b) => {
          ...
          b.style.borderColor = bg === _currentColor ? "#fff" : bg;
        });
        bar.querySelectorAll("[data-tracebug='draw-color-btn']").forEach((b) => {
  ...
  function updateToolbarRecordingState(isRecording2) {
    ...
    if (dot) {
      dot.style.background = isRecording2 ? "#22c55e" : "#ef4444";
  ...
  function mountCompactToolbar(root, panel, showToast2, renderAnnotationList2) {
    _panelEl = panel;
    ...
    const annotateBtn = _createToolbarBtn(
      ...
      () => {
        if (isElementAnnotateActive()) {
          ...
          _updateActiveStates(toolbar);
          showToast2("Annotate mode: Click elements to annotate. Shift+click for multi-select.", root);
        }
      },
    ...
    const drawBtn = _createToolbarBtn(
      ...
      () => {
        if (isDrawModeActive()) {
          ...
          _updateActiveStates(toolbar);
          showToast2("Draw mode: Drag to draw rectangles or ellipses.", root);
        }
      },
    ...
    <div style="padding:16px 20px;border-bottom:1px solid #2a2a3e;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
      <div>
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div id="bt-rec-indicator" style="width:8px;height:8px;border-radius:50%;background:${_isRecording2 ? "#22c55e" : "#ef4444"};animation:${_isRecording2 ? "bt-pulse 2s infinite" : "none"}" title="${_isRecording2 ? "Recording" : "Paused"}"></div>
        </div>
...
<div style="text-align:center;color:#333;font-size:11px;margin-top:32px;padding:16px;border-top:1px solid #1e1e32">Generated by TraceBug AI \xB7 ${(/* @__PURE__ */ new Date()).toLocaleString()}</div>
  ...
  function renderAnnotationList(panel) {
    ...
    <div style="padding:16px 20px;border-bottom:1px solid #2a2a3e;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
      <div>
        ...
        <div style="font-size:11px;color:#666;margin-top:2px">${elAnnotations.length} element${elAnnotations.length !== 1 ? "s" : ""} \xB7 ${drawRegions.length} region${drawRegions.length !== 1 ? "s" : ""}</div>
      </div>
      <div style="display:flex;gap:6px">
  ...
  `;
    ...
    panel.querySelector("#bt-ann-export-md").addEventListener("click", async () => {
      ...
      setTimeout(() => btn.textContent = "MD", 2e3);
    });
    panel.querySelector("#bt-ann-export-json").addEventListener("click", async () => {
      ...
      btn.textContent = ok ? "Copied!" : "Failed";
      setTimeout(() => btn.textContent = "JSON", 2e3);
    });
    ...
    if (elAnnotations.length > 0) {
      ...
      for (let i = 0; i < elAnnotations.length; i++) {
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
          ...
          <span style="font-size:10px;padding:1px 6px;border-radius:3px;background:${intentColor}22;color:${intentColor};border:1px solid ${intentColor}44;font-weight:600;text-transform:uppercase">${a2.intent}</span>
          <span style="font-size:10px;padding:1px 6px;border-radius:3px;background:${sevColor}22;color:${sevColor};border:1px solid ${sevColor}44;text-transform:uppercase">${a2.severity}</span>
          <button class="bt-ann-delete" data-id="${a2.id}" style="margin-left:auto;background:none;border:none;color:#555;cursor:pointer;font-size:12px;padding:2px" title="Delete">x</button>
        </div>
        <div style="font-size:11px;color:#888;margin-bottom:4px;font-family:monospace">&lt;${escapeHtml2(a2.tagName)}&gt; ${escapeHtml2(a2.innerText.slice(0, 50))}</div>
        <div style="font-size:11px;color:#e0e0e0;line-height:1.4">${escapeHtml2(a2.comment)}</div>
        <div style="font-size:9px;color:#444;margin-top:4px">${escapeHtml2(a2.page)} \xB7 ${escapeHtml2(a2.selector.slice(0, 50))}</div>
      </div>`;
      }
    }
    if (drawRegions.length > 0) {
      ...
      for (let i = 0; i < drawRegions.length; i++) {
        ...
        <div style="font-size:11px;color:#e0e0e0;line-height:1.4">${r.comment ? escapeHtml2(r.comment) : '<span style="color:#444">No comment</span>'}</div>
        <div style="font-size:9px;color:#444;margin-top:4px">${escapeHtml2(r.page)}</div>
      </div>`;
      }
    }
    content2.innerHTML = html;
