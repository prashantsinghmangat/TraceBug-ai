...
`;
  ...
  `;
    ...
    ${data.screenshot ? `
      ...
      <div style="font-size:10px;color:var(--tb-text-muted, #555);margin-bottom:16px">${escapeHtml2(data.screenshot.filename)} \xB7 ${data.screenshot.width}x${data.screenshot.height}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
      ...
      <button data-action="jira" style="background:#2684FF;color:#fff;border:none;border-radius:var(--tb-radius-md, 6px);padding:12px;cursor:pointer;font-size:13px;font-weight:600;font-family:inherit;transition:opacity 0.15s">\u{1F3AB} Copy as Jira Ticket</button>
    </div>
...
_Screenshot attached: ${screenshot.filename}_` : ""}`;
  ...
  var _githubRepo, MODAL_ID, DRAFT_KEY, _isOpen;
  var init_quick_bug = __esm({
    "src/ui/quick-bug.ts"() {
      ...
      init_helpers();
      _githubRepo = null;
      MODAL_ID = "tracebug-quick-bug-modal";
    ...
    toolbar.appendChild(_createToolbarBtn(
      ...
      async () => {
        ...
        try {
          ...
          showToast3(`Screenshot saved: ${ss.filename}`, root);
        } catch (e2) {
